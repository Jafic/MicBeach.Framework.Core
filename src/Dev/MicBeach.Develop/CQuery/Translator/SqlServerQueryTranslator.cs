    ...
    public class SqlServerQueryTranslator : IQueryTranslator
    {
        ...
        public TranslateResult ExecuteTranslate(IQuery query, Dictionary<string, object> paras = null, string objectName = "", bool subQuery = false)
        {
            ...
            if (query.QueryType == QueryCommandType.QueryObject)
            {
                ...
                if (query.RecurveCriteria != null)
                {
                    ...
                    {
                        preScript = string.Format("{0} WHERE {1}.[{2}]={3}.[{4}])", preScript, secondObjectPetName, query.RecurveCriteria.Key, firstTreeTablePetName, query.RecurveCriteria.RelationKey);
                    }
                    else
                    {
                        preScript = string.Format("{0} WHERE {1}.[{2}]={3}.[{4}])", preScript, secondObjectPetName, query.RecurveCriteria.RelationKey, firstTreeTablePetName, query.RecurveCriteria.Key);
                    }
                }
        ...
        string TranslateCriteria(Criteria criteria, Dictionary<string, object> parameters, string objectName)
        {
            ...
            if (valueQuery != null)
            {
                ...
                return string.Format("{0} {1} (SELECT {2} {3}.[{4}] FROM [{5}] {6} {7} {8})", ConvertCriteriaName(objectName, criteria), sqlOperator, topString, subObjName, valueQuery.QueryFields[0], valueQuery.ObjectName, subObjName, conditionString, subQueryResult.OrderString);
            }
